...
simulate_lineage <- function(Q.list,
  ...
  # -------- init cells (hg model + sg model) --------
  list_hg <- list()
  list_sg <- list()
  
  for (i in 1:cell.num) {
    status.matrix <- matrix(1, nrow = gRNA.num + 3, ncol = 1)
    status.matrix[gRNA.num + 1, 1] <- rposnorm(1, div_mean, div_sd) * runif(1, 0, 1) # next division
    status.matrix[gRNA.num + 2, 1] <- 0                                              # quiesce_end
    status.matrix[gRNA.num + 3, 1] <- rexp(1, rate = death_rate_per_time)            # death time
  ...
  for (time in 0:t_max) {
    ...
    for (i in 1:length(list_hg)) {
      # current state (last column)
      ...
      if (time >= next_div_time) {
        ...
        # hg: full state (barcode + 3 timers)
  ...
  # return terminal states too (so you can label tips etc.)
  terminal_hg <- lapply(list_hg, function(m) m[, ncol(m)])
  terminal_sg <- lapply(list_sg, function(m) m[, ncol(m)])
  
  cell_names <- paste0("C", seq_along(list_hg))
  
  names(list_hg) <- cell_names
  names(list_sg) <- cell_names
  
  names(terminal_hg) <- cell_names
  names(terminal_sg) <- cell_names
